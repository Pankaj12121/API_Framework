...
public class UserDetails extends WebPage {
	...
	public void extServicesUserDetailsStep(XSSFWorkbook workbook, String sheetName, ExcelOperation excelOperation,
			String scenarioID) throws Exception {
		try {
			String uri = ServiceEndpoint.userDetails.getUrl();
			...
			Response res1 = RestAssured.given().header("Authorization", TestEngine.sessionID).when().body(nJson)
			...
			JsonObject NewJson2 = NewJson.get("data").getAsJsonObject();
			String clientID = NewJson2.get("clientID").getAsString();
			String userName = NewJson2.get("userName").getAsString();
			String exchange = NewJson2.get("exchange").getAsString();
			String product = NewJson2.get("product").getAsString();
			String broker = NewJson2.get("broker").getAsString();
			String branch = NewJson2.get("branch").getAsString();

			assertBlank(clientID, "clientID");
			assertBlank(userName, "userName");
			assertBlank(exchange, "exchange");
			assertBlank(product, "product");
			assertBlank(broker, "broker");
			assertBlank(branch, "branch");
